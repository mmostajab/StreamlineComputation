...
StreamTracer::StreamTracer()
{
  ...
  m_surface_parameters.traceIntegrationMethod = SurfaceParameters::IM_CASHKARP;
...
glm::dvec3 StreamTracer::step(UnstructDataset* dataset, const glm::dvec3& p0, const glm::dvec3& deriv, double& stepsize, int& stepsizeAdapted)
{
  ...
  if (m_surface_parameters.traceIntegrationMethod == SurfaceParameters::IntegrationMethod::IM_EULER)
  {
    p1 = p0 + stepsize * deriv;
	  return p1;
  }
  else if (m_surface_parameters.traceIntegrationMethod == SurfaceParameters::IntegrationMethod::IM_CASHKARP)
  {
    stepsizeAdapted = 0;
